...
# output each samples gt probabilities for debug
def Run(args):
    ...
    if args.input_probabilities:
        call_variants_with_probabilities_input(args=args, output_config=output_config, output_utilities=output_utilities)
    elif args.output_probabilities:
        predict(args=args, output_config=output_config, output_utilities=output_utilities,platform=args.platform)


def Run_call_trio(args):
    os.environ["OMP_NUM_THREADS"] = "1"
    os.environ["OPENBLAS_NUM_THREADS"] = "1"
    os.environ["MKL_NUM_THREADS"] = "1"
    os.environ["NUMEXPR_NUM_THREADS"] = "1"
    ...
    output_utilities_c = output_utilties_class(
        sample_name=args.sampleName_c,
        is_debug=args.debug,
        is_output_for_ensemble=args.output_for_ensemble,
        reference_file_path=args.ref_fn,
        output_file_path=args.call_fn_c,
        output_probabilities=args.output_probabilities
    )


    # parent1
    output_config_p1 = OutputConfig(
        is_show_reference=args.showRef,
        is_debug=args.debug,
        is_haploid_precise_mode_enabled=args.haploid_precise,
        is_haploid_sensitive_mode_enabled=args.haploid_sensitive,
    ...
    output_utilities_p1 = output_utilties_class(
        sample_name=args.sampleName_p1,
        is_debug=args.debug,
        is_output_for_ensemble=args.output_for_ensemble,
        reference_file_path=args.ref_fn,
        output_file_path=args.call_fn_p1,
        output_probabilities=args.output_probabilities
    )

    # parent2
    #output_config_p2 = OutputConfig(
    #    is_show_reference=args.showRef,
    #    is_debug=args.debug,
    #    is_haploid_precise_mode_enabled=args.haploid_precise,
    #    is_haploid_sensitive_mode_enabled=args.haploid_sensitive,
    ...
    #output_utilities_p2 = output_utilties_class(
    #    sample_name=args.sampleName_p2,
    #    is_debug=args.debug,
    #    is_output_for_ensemble=args.output_for_ensemble,
    #    reference_file_path=args.ref_fn,
    #    output_file_path=args.call_fn_p2,
    #    output_probabilities=args.output_probabilities
    #)

    call_variants(args=args, \
        output_config_c=output_config_c, output_utilities_c=output_utilities_c, \
        output_config_p1=output_config_p1, output_utilities_p1=output_utilities_p1, \
        )#output_config_p2=output_config_p2, output_utilities_p2=output_utilities_p2)

class output_utilties_class:
    def __init__(self, output_file_path, sample_name, is_debug, is_output_for_ensemble, reference_file_path, output_probabilities):
        self.output_file = open(output_file_path, "w")
        self.sample_name = sample_name
        self.is_debug = is_debug
        self.is_output_for_ensemble = is_output_for_ensemble
        self.reference_file_path = reference_file_path
        self.output_probabilities = output_probabilities

    def output(self, string_value):
        print(string_value, file=self.output_file)
...
def batch_output(batch_chr_pos_seq, alt_info_list, batch_Y, output_config, output_utilities):
    ...
    #_tar_cum = [0] + param.label_shape_cum_trio
    _tar_cum = [0] + param.label_shape_cum_dual
    _tar_cum_off = 0
    if output_config.model_arc != "N1":
        if output_config.trio_n_id == 1:
            _tar_cum_off = 4
        elif output_config.trio_n_id == 2:
            _tar_cum_off = 8

    batch_gt21_probabilities, batch_genotype_probabilities = batch_Y[: , _tar_cum[_tar_cum_off + 0]:_tar_cum[_tar_cum_off + 1]], batch_Y[: , _tar_cum[_tar_cum_off + 1]:_tar_cum[_tar_cum_off + 2]]
    if len(batch_gt21_probabilities) != batch_size:
        sys.exit(
            "Inconsistent shape between input tensor and output predictions %d/%d" %
            (batch_size, len(batch_gt21_probabilities))
    ...
    if output_config.add_indel_length:
        batch_variant_length_probabilities_1, batch_variant_length_probabilities_2 = \
            batch_Y[: , _tar_cum[_tar_cum_off + 2]:_tar_cum[_tar_cum_off + 3]], batch_Y[: , _tar_cum[_tar_cum_off + 3]:_tar_cum[_tar_cum_off + 4]]

    # print( _tar_cum[_tar_cum_off + 0], _tar_cum[_tar_cum_off + 1], _tar_cum[_tar_cum_off + 2], _tar_cum[_tar_cum_off + 3], _tar_cum[_tar_cum_off + 4])
...
def output_with(
    ...
    ori_alt_info = alt_info

    alt_info = alt_info.rstrip().split('\t')
    alt_info_c = alt_info[0].split('-') if len(alt_info) > 1 else ''
    alt_info_p1 = alt_info[1].split('-') if len(alt_info) > 2 else ''
    #alt_info_p2 = alt_info[2].split('-') if len(alt_info) == 3 else ''

    if output_config.trio_n_id == 0:
        alt_info = alt_info_c
    elif output_config.trio_n_id == 1:
        alt_info = alt_info_p1
    #elif output_config.trio_n_id == 2:
    #    alt_info = alt_info_p2
    else:
        return
    #print(position, alt_info, output_config.trio_n_id)
    #import pdb; pdb.set_trace()

    if alt_info == '':
        # print('[W], in site %s:%s, alt info incomplete, alt number [%d], %s' % (chromosome, position, len(ori_alt_info.rstrip().split('\t')), ori_alt_info[:30]))
        return
...
def call_variants(args, output_config_c, output_utilities_c, \
                        output_config_p1, output_utilities_p1 \
    ...
    label_size = param.label_size_dual
    m.load_weights(args.chkpnt_fn)

    #output_utilities_c.gen_output_file()
    output_utilities_c.output_header()

    #output_utilities_p1.gen_output_file()
    output_utilities_p1.output_header()

    #output_utilities_p2.gen_output_file()
    #output_utilities_p2.output_header()
    ...
    while True:
        ...
        if len(mini_batches_to_output) > 0:
            ...
            thread_pool.append(Thread(
                ...
                args=(position, alt_info_list, prediction, output_config_c, output_utilities_c)
            ))
            thread_pool.append(Thread(
                target=batch_output_method,
                args=(position, alt_info_list, prediction, output_config_p1, output_utilities_p1)
            ))
            #thread_pool.append(Thread(
            #    target=batch_output_method,
            #    args=(position, alt_info_list, prediction, output_config_p2, output_utilities_p2)
            #))
        ...
        if is_finish_loaded_all_mini_batches and is_nothing_to_predict_and_output:
            break
    try:
        chunk_infos = args.call_fn_c.split('.')[-2]
    ...
    if total == 0:
        #logging.info(log_error("[WARNING] empty trio-alignment output for file {} {}".format(args.ctgName, args.call_fn_c)))
        logging.info("[WARNING] empty trio-alignment output for file {} {}".format(args.ctgName, args.call_fn_c))
...
# using trained model to predict 21gt, etc.
def predict(args, output_config, output_utilities,platform):
    print(platform)
    chunk_id = args.chunk_id - 1 if args.chunk_id else None  # 1-base to 0-base
    chunk_num = args.chunk_num
    ...
    if args.model_arc == "N1":
        from mp.model import Clair3_Trio_Basic
        m = Clair3_Trio_Basic(add_indel_length=args.add_indel_length, predict=True, is_padding=args.add_padding)
        label_size = param.label_size

        print('target output is [%s] %d' % (['child', 'p1', 'p2'][output_config.trio_n_id], output_config.trio_n_id))
        if output_config.trio_n_id == 2:
            print('will reverse tesnor p1 and p2 order')

    else:
        if args.model_cls == "Clair3_Trio_Out3":
            m = model_path.Clair3_Trio_Out3(add_indel_length=args.add_indel_length, predict=True)
        elif args.model_cls == "Clair3_Trio_V_res":
            m = model_path.Clair3_Trio_V_res(add_indel_length=args.add_indel_length, predict=True)
        elif args.model_cls == "Clair3_Trio_V_rres":
            m = model_path.Clair3_Trio_V_rres(add_indel_length=args.add_indel_length, predict=True)
        elif args.model_cls == "Clair3_Trio_V_o1":
            m = model_path.Clair3_Trio_V_o1(add_indel_length=args.add_indel_length, predict=True)
        elif args.model_cls == "Clair3_Trio_V_o2":
            m = model_path.Clair3_Trio_V_o2(add_indel_length=args.add_indel_length, predict=True)
        elif args.model_cls == "Clair3_Trio_V_o1_rres":
            m = model_path.Clair3_Trio_V_o1_rres(add_indel_length=args.add_indel_length, predict=True)
        else:
            raise ValueError('Unsupported model_cls name: ',args.model_cls)
        logging.info("Model class name: %s" % (args.model_cls))

        # from trio.model import Clair3_Trio_Out3
        # m = Clair3_Trio_Out3(add_indel_length=args.add_indel_length, predict=True) 
        #label_size = param.label_size_trio
        label_size = param.label_size_dual
    # loadx model
    m.load_weights(args.chkpnt_fn)
    ...
    if not args.is_from_tables:
        ...
        tensor_shape = param.ont_input_shape_dual
        if platform == 'hifi':
            tensor_shape = param.pb_ilmn_input_shape_dual
        if args.add_padding:
            #tensor_shape = param.p_ont_input_shape_trio
            tensor_shape = param.p_ont_input_shape_dual
            if platform == 'hifi':
                tensor_shape = param.p_pb_ilmn_input_shape_dual
        # process by chunk windows
        if chunk_id is not None and chunk_num is not None:
            chunk_dataset_size = dataset_size // chunk_num if dataset_size % chunk_num == 0 else dataset_size // chunk_num + 1
            chunk_start_pos = chunk_id * chunk_dataset_size
        ...
        # legacy, may configure to have trio output
        ...
        position_memmap = np.lib.format.open_memmap(predict_fn + '.position', dtype='S%d' % (param.no_of_positions + 50), mode='w+',
                                                    shape=(dataset_size, 1))
        alt_info_memmap = np.lib.format.open_memmap(predict_fn + '.alt_info', dtype='S%d' % (param.max_alt_info_length), mode='w+',
                                                    shape=(dataset_size, 1))
        ...
        for idx in range(num_epoch):
            ...
            if args.model_arc == "N1" and output_config.trio_n_id == 2:
                # print(output_config.trio_n_id)
                one_tensor_shape = int(tensor_shape[0] / 3)
                # reverse p1 and p2 tensor order in model's input
                new_position_matrix = tf.concat([position_matrix[:, one_tensor_shape * 0 : one_tensor_shape * 1,:,:], \
...
def main():
    ...
    parser.add_argument('--trio_n_id', type=int, default=0,
                        help="target trio sample id, [0, 1, 2] for [child, parent1, parent2], default: %(default)s")
    ...
    parser.add_argument('--add_indel_length', type=str2bool, default=False, help=SUPPRESS)
    ...
    parser.add_argument('--model_cls', type=str, default="Clair3_Trio_Basic",
    ...
    if args.input_probabilities or args.output_probabilities:
        Run(args)
    else:
        Run_call_trio(args)
