...
def Run(args):
    ...
    table_file.create_earray(where='/', name='label', atom=int_atom, shape=(0, label_size_trio), filters=utils.FILTERS)
    ...
    for f in in_fn_list:
        ...
        for index in range(len(fi.root.label)):
            ...
            if total_compressed % 500 == 0 and total_compressed > 0:
                table_dict = utils.write_table_file(table_file, table_dict, tensor_shape_trio, label_size_trio, float_type)
