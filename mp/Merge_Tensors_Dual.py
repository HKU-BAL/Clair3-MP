...
def Merge_tensors(args):
    ...
    logging.info("%s: trio sites [%d, %d], valid %d/%d (%.2f%%)" % (_trio_info_n, len(can_c), \
        len(can_p1), _trio_s, _c_s, 100.*_trio_s/_c_s))
    ...
    while True:
        ...
        if len(_info_c) > 0:
            ...
            #_hit_l = (len(_info_c), len(_info_p1))
            ...
            alt_fp.write('\t'.join([ctg_name + ' ' + str(pos), alt_info_c, alt_info_p1]) + '\n')
            # import pdb; pdb.set_trace()
        else:
            break
