...
def tensor_generator_from(tensor_file_path, batch_size, pileup, platform):
    ...
    tensor_shape_trio = param.ont_input_shape_dual

    if platform == 'hifi':
        #print("haha")
        tensor_shape = param.pb_ilmn_input_shape_dual
        tensor_shape_one = param.input_shape
        tensor_shape_trio = param.pb_ilmn_input_shape_dual
    #print(tensor_shape_trio)
    ...
    def item_from(row):
        ...
        chrom, coord, seq, string_c, alt_info_c, string_p1, alt_info_p1 = row.rstrip().split("\t")
        ...
        pos = chrom + ":" + coord + ":" + seq
        alt_info = '%s\t%s' % (alt_info_c, alt_info_p1)
...
def write_table_dict(table_dict, pos, \
    ...
    table_dict['alt_info'].append('%s\t%s' % (alt_info_c, alt_info_p1))
...
def bin_reader_generator_from(subprocess_list, Y_true_var_c, Y_true_var_p1, Y_c, Y_p1, \
    is_tree_empty, tree, miss_variant_set_c, miss_variant_set_p1, \
    truth_alt_dict_c, truth_alt_dict_p1, \
    ...
    for f in subprocess_list:
        for row_idx, row in enumerate(f.stdout):
            ...
            if key not in X:
                ...
                X[key] = (string_c, alt_info_c, string_p1, alt_info_p1, seq)
                if is_reference:
                    # import pdb; pdb.set_trace()
                    ref_list.append(key)
                ...
                if len(new_key) > 0:
                    ...
                    X[new_key] = (string_c, alt_info_c, string_p1, alt_info_p1, seq)
                if is_reference:
                    # import pdb; pdb.set_trace()
...
def get_training_array(tensor_fn, var_fn_c, var_fn_p1, bed_fn, bin_fn, shuffle=True, is_allow_duplicate_chr_pos=True, chunk_id=None,
    ...
    if add_padding:
        ...
        print("[INFO] add padding tensors %s, %s" % \
            (param.padding_value_c, param.padding_value_p1), file=sys.stderr)
        tensor_shape_trio = param.p_ont_input_shape_dual
        if platform == 'hifi':
             tensor_shape_trio = param.p_pb_ilmn_input_shape_dual
    # import pdb; pdb.set_trace()

    label_size_one = param.label_size
    label_size_trio = param.label_size_dual
    ...
    while True:
        ...
        for key in all_chr_pos:
            string_c, alt_info_c, string_p1, alt_info_p1, seq = X[key]
