...
def FiterHeteSnp(args):
    ...
    if args.for_train:
        ...
        print('in %s, read true %d, %d, union %d' % (contig_name, len(Y_c), len(Y_p1), len(Y_all)))
    ...
    can_set_all = can_set_c | can_set_p1
    ...
    _ori_ref_n = int(sum([len(lq_can_c[0]), len(lq_can_p1[0])]) / 2)
    _ori_v_n = int(sum([len(lq_can_c[1]), len(lq_can_p1[1])]) / 2)
    ...
    ref_set_all = ref_set_c | ref_set_p1
    ...
    _tmp_ref_set_all = _tmp_ref_set_c | _tmp_ref_set_p1
    ...
    if args.get_subtract:
        ...
        tmp_s_v = set(can_set_p1) - can_set_c
        tmp_s_r = set(ref_set_p1) - ref_set_c
